...
def annotation_parser(response: dict) -> dict:
    ...
    for match in matches:
        try:
            sample_id = int(match[0])
            # 標準化 label
            label = str(match[1]).strip().capitalize()
            if label in ['Yes', 'No']:
                 predictions.append({'id': sample_id, 'prediction': label}) # 注意，這裡應該更新 'annotation' 欄位
            else:
                 print(f"[Warning] Parsed an invalid label '{label}' for ID {sample_id}. Skipping.")
        except (ValueError, IndexError):
            print(f"[Warning] Failed to parse match: {match}")
            continue
